...
public class DragImage extends JComponent implements MouseMotionListener 
{
  ...
  public void paint(Graphics g) {
    ...
    // Методы g2.drawImage(...), image.getWidth(...), image.getHeight(...) являются
    // асинхронными и подразумевают, что изображение может быть еще не загружено из источника 
    // к данному моменту. В таком случае, drawImage рисует только ту часть изображения, которая загружена;
    // а getWidth и getHeight возвращают -1. 
    // Для получения событий о прогрессе/окончании загрузки изображения используется callback-интерфейс
    // ImageObserver. Мы в данном случае используем сам компонент DragImage в качестве ImageObserver.
    // (по-умолчанию компонент будет вызывать свой метод repaint при обновлении статуса загрузки изображения)
  }
